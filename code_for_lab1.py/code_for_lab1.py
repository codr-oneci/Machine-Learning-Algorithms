...
def xval_learning_alg(learner, data, labels, k):
    scor_total=0
    taieturi=np.array_split(data,k,axis=1)
    puncte=np.array_split(labels,k,axis=1)
    lista_date=[]
    lista_label=[]
    for i in range(len(taieturi)):
        lista_date.append(())
        lista_label.append(())
    for j in range(k):
        for i in range(k):
            if i!=j:
                lista_date[j]+=(taieturi[i],)
                lista_label[j]+=(puncte[i],)
        scor_total+=eval_classifier(learner, np.concatenate(lista_date[j],axis=1),np.concatenate(lista_label[j],axis=1),taieturi[j],puncte[j])
    return scor_total/k
...
def perceptron(data, labels, params={}, hook=None):
    ...
    (d,n)=np.shape(data)
    theta_0=np.reshape(np.zeros(1),(1,1))
    theta=np.zeros((d,1))
    lista = []
    for i in range(1, n + 1):
        lista.append(i)
    taieturi = np.split(data, lista, axis=1)
    puncte=np.split(labels,lista,axis=1)

    for t in range(1,T+1):
        for i in lista:
            j=i-1
            if puncte[j]*y(taieturi[j],theta,theta_0) <= 0:
                theta+=puncte[j]*taieturi[j]
                theta_0+=puncte[j]
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    theta_0 = np.reshape(np.zeros(1), (1, 1))
    theta = np.zeros((d, 1))
    theta_0_average = np.reshape(np.zeros(1), (1, 1))
    ...
    for t in range(1, T + 1):
        for i in lista:
            ...
            if puncte[j] * y(taieturi[j], theta, theta_0) <= 0:
                theta += puncte[j] * taieturi[j]
                theta_0 += puncte[j]
                theta_average += c*puncte[j] * taieturi[j]
                theta_0_average+=c*puncte[j]
            c-=1/(n*T)
    return (theta_average, theta_0_average)


def eval_learning_alg_on_training(learner, data_gen, n_train, it):
    score=0
    for iteratie in range(1,it+1):
        data_train,labels_train=data_gen(n_train)
        score+=eval_classifier(learner,data_train,labels_train,data_train,labels_train)
    return score/it
print(eval_learning_alg_on_training(averaged_perceptron,gen_flipped_lin_separable,20,1000))